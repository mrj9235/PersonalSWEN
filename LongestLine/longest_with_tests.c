...
int main( int argc, char *argv[] ) {		// LLK addition for unit testing
	...
	char copyline[MAXLINE + 1];  //copied input line
	int max = -1;	//keeps track of the longest length
...
int readline( char s[], int max_chars ) {
	/* your code here */
	int c = getchar(); //initializes c to the first character of the line
	int i = 0; //counter
	int len = 0; //initializes the length of the string to 0
	while( c != '\n'){
		if(i < max_chars){
			if( c != EOF){
				s[i] = c; //changes the place 'i' in the array to equal c
		...
		len ++; //adds 1 to lenght
		c = getchar(); //sets c to the next character from the input line
	}
	s[i] = '\0'; // ends the string
	len ++;
	return len; // placeholder for the real return value.
}
...
void copy( char to[], char from[] ) {
	...
	for( i = 0; from[i] != '\0'; i++){ //for loops that goes through from[] until from [i] == '\0'
		to[i] = from[i]; // sets the coinciding location in to[] equal to the same spot from from[]
	}
	to[i] = '\0';
}
